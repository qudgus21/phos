...
이 섹션은 실제 theme-picker 플로우를 여러 차례 실행하면서 배운 교훈이다.

### 1. 분석 → HTML 생성까지 끊지 마라
- 사용자는 중간 분석 결과를 볼 필요 없다
- Phase 0 분석이 끝나면 바로 Phase 2 HTML 생성까지 한번에 진행
- 사용자의 첫 번째 인터랙션은 "프리뷰를 보고 번호를 선택"해야 함

### 2. Grep 도구의 한계를 알아두라
- `Grep` 도구가 프로젝트 내 `.tsx`/`.css` 파일에서 `emerald`, `#10B981` 등을 못 찾는 경우가 있다
- **반드시 `Bash`의 `grep -rn` 명령을 폴백으로 준비**하라
- 특히 `rgba(` 같은 특수문자 포함 패턴은 Bash grep이 더 확실하다

### 3. 색상 교체는 2계층을 모두 처리해야 한다
- **CSS 변수 계층**: `globals.css`의 `--primary` 등 → 이것만 바꾸면 `text-primary`, `bg-primary` 등은 자동 전파
- **Tailwind 하드코딩 계층**: `from-emerald-500`, `text-emerald-400` 등 → 이건 파일마다 개별 수정 필요
- 하나만 바꾸면 색상이 불일치한다

### 4. `replace_all: true`를 적극 활용하라
- 같은 파일 내에서 동일한 rgba 패턴이 여러 곳에 나올 수 있다
- `replace_all: true`로 한번에 전부 교체하면 누락 위험이 없다

### 5. 현재 팔레트 옵션은 필수
- 사용자가 비교 후 "현재가 더 나은데" 할 수 있다
- 1번 옵션은 항상 현재 적용 중인 팔레트로 고정하고 `(현재 적용)` 라벨을 붙인다

### 6. 빌드 검증 + 잔여 참조 확인은 필수
- `npm run build` 성공 확인
- 이전 색상의 잔여 참조를 `grep`으로 확인 (0건이어야 함)
- 잔여가 있으면 누락된 파일이 있다는 뜻

### 7. 버튼 그라디언트는 동일 색상 계열 내에서만
- **크로스 컬러 그라디언트 (예: emerald→violet, 초록→보라)는 과하다** — 사용자가 "눈이 아프다"고 피드백
- 올바른 방식: **같은 색상 계열의 명도 차이**로 그라디언트 표현
  - 예: `from-indigo-600 to-indigo-400` (어두운 인디고 → 밝은 인디고)
  - 예: `from-rose-600 to-rose-400`
- 이렇게 하면 깊이감은 있되 시각적으로 과하지 않음
- "그라디언트 빼라"는 뜻이 아니라 "절제하라"는 뜻임을 이해할 것

### 8. 버튼 색상 일관성 필수
- 같은 역할의 버튼(CTA)은 모든 섹션에서 **동일한 스타일**이어야 한다
- 적용 시 체크리스트:
  - `button.tsx`의 `glow` variant
  - `hero.tsx`의 CTA Link
  - `skin-realism.tsx`의 CTA Link
  - `pricing.tsx`의 추천 플랜 CTA Link
  - `cta-section.tsx`의 CTA Link
- 모든 CTA가 동일한 `from-{color}-600 to-{color}-400` + `btn-glow` + `hover:brightness-110` 패턴을 사용해야 함

### 9. gradient-text와 slider-line은 미세한 그라디언트 OK
- `.gradient-text`와 `.slider-line`은 텍스트/장식 요소이므로 primary→밝은 shade 그라디언트가 적절
  - 예: `linear-gradient(135deg, #6366F1, #818CF8)` (indigo-500 → indigo-400)
- 버튼처럼 큰 면적에서 강한 크로스 컬러 그라디언트는 피하지만, 텍스트나 가는 선에는 같은 계열 내 변화 OK

### 10. 사용자 피드백은 "정도"의 문제
- "그라디언트 과하다" = 완전 제거가 아니라 톤 다운
- "버튼 밝아서 눈 아프다" = 배경색 자체를 어둡게가 아니라 대비와 glow 강도 조절
- 극단적 해석 말고, 사용자 의도의 방향성을 파악할 것
